...
App = {
  ...
  fillOutForm(id) {
    ...
    tags.forEach(tag => {
      if(tag) $(form).find(`[value=${tag}]`).prop('checked', true);
    });
  },
  ...
  updateContact(form, data) {
    const id = $('button#update')[0].dataset.id;
...
Contacts = {
  ...
  matchQuery(obj) {
    const query = App.queryInput;
    const regexStartWithQuery = new RegExp(`^${query}|\\s${query}`, 'i');
